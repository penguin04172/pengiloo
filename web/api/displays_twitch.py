...
router = APIRouter(prefix='/displays/twitch', tags=['displays'])


@router.get('')
async def twitch_display(request: Request, display_id: str = '', nickname='') -> dict:
    path = await enforce_display_configuration(request, display_id, nickname, {'channel': ''})
    if path is not None:
