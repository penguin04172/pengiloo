...
class MatchOut(Match):
    ...
    def is_complete(self) -> bool:
        return self.status == MATCH_STATUS.red_won_match or self.status == MATCH_STATUS.blue_won_match or self.status == MATCH_STATUS.tie_match
...
def read_matches_by_type(match_type: MATCH_TYPE, include_hidden: bool = False):
    ...
    return [MatchOut(**m.to_dict()) for m in matches if m.type == match_type and (include_hidden or m.status != MATCH_STATUS.match_hidden)]
