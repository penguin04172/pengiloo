...
from .game_specific import TIME_CONSTANT, specific
...
class AmpSpeaker(BaseModel):
    ...
    def update_state(
        ...
        auto_validity_cutoff = match_start_time + timedelta(
            seconds=timing.get_duration_to_auto_end() + TIME_CONSTANT.speaker_auto_grace_period_sec
        )
        if current_time < auto_validity_cutoff:
            self.auto_amp_notes += new_amp_notes
            self.banked_amp_notes = min(
                self.banked_amp_notes + new_amp_notes, specific.BANKED_AMP_NOTE_LIMIT
            )
        ...
        teleop_amp_validaty_cutoff = match_start_time + timedelta(
            seconds=timing.get_duration_to_teleop_end()
        )
    ...
    def amplified_time_remaining(self, current_time: timedelta) -> float:
        time_left = timedelta(seconds=specific.amplification_duration_sec) - (
    ...
    def is_coop_window_open(self, match_start_time: timedelta, current_time: timedelta) -> bool:
        coop_validity_cutoff = (
            match_start_time
            + timedelta(seconds=timing.get_duration_to_teleop_start())
            + timedelta(seconds=TIME_CONSTANT.coop_teleop_window_sec)
        )
        return specific.melody_bonus_threshold_with_coop > 0 and current_time < coop_validity_cutoff
    ...
    def is_amplified(self, current_time: timedelta, include_grace_period: bool) -> bool:
        amplified_validity_cutoff = self.last_amplified_time + timedelta(
            seconds=specific.amplification_duration_sec
        )
        ...
        meets_time_criterion = (
            ...
            and current_time > timedelta(seconds=timing.get_duration_to_auto_end())
        )
        meets_note_criterion = (
            specific.amplification_note_limit == 0
            or self.last_amplified_speaker_notes < specific.amplification_note_limit
        )
