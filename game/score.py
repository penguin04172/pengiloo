...
class Score(BaseModel):
    ...
    def summarize(self, opponentScore: "Score") -> ScoreSummary:
        ...
        summary.stage_points = summary.park_points + summary.onstage_points + summary.harmony_points + summary.spotlight_points + summary.trap_points
        summary.match_points = summary.leave_points + summary.amp_points + summary.speaker_points + summary.stage_points

        # Calculate penalty points
        for foul in opponentScore.fouls:
        ...
        if game_specific.melody_bonus_threshold_with_coop > 0:
            ...
            summary.coopertition_bonus = summary.coopertition_criteria_met and opponentScore.amp_speaker.coop_activated
        ...
        if summary.stage_points >= game_specific.ENSEMBLE_BONUS_POINT_THRESHOLD and total_onstage_robots >= game_specific.ENSEMBLE_BONUS_ROBOT_THRESHOLD:
    ...
    def equals(self, other: "Score") -> bool:
        if self.leave_statuses != other.leave_statuses or \
        self.amp_speaker != other.amp_speaker or \
        self.endgame_statuses != other.endgame_statuses or \
        self.microphone_statuses != other.microphone_statuses or \
        self.trap_statuses != other.trap_statuses or \
        self.playoff_dq != other.playoff_dq or \
        len(self.fouls) != len(other.fouls):
            return False
